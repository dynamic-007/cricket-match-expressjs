...
app.get("/players/", async (request, response) => {
  ...
  response.send(Response.map((eachPlayer) => CPDBO(eachPlayer)));
});
...
app.get("/players/:playerId/", async (request, response) => {
  ...
  response.send(CPDBO(getPlayerByIDResponse));
});
...
app.put("/players/:playerId/", async (request, response) => {
  ...
  const updateQ = `
    ...
    WHERE player_id=${playerId}`;
  const updatePlayerNameResponse = await db.run(updateQ);
...
app.get("/matches/:matchId/", async (request, response) => {
  ...
  response.send(convertMatch(getMatchDetailsResponse));
});
...
app.get("/players/:playerId/matches", async (request, response) => {
  ...
  response.send(
    fetchMatchDetailsResponse.map((eachMatch) => convertMatch(eachMatch))
  );
});
...
app.get("/matches/:matchId/players", async (request, response) => {
  ...
  response.send(
    getPlayersOfMatchResponse.map((eachPlayer) => CPDBO(eachPlayer))
  );
});
